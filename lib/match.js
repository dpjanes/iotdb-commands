...
const match = function(paramd, done) {
    ...
    const _level_2 = function(thingd) {
        ...
        const thing = iotdb_thing.make(thingd);

        let match_ad = _.flatten(
            [   ads.find(ad => thing.attribute(_strip_non_colon(ad.istate))), 
                ads.find(ad => thing.attribute(_strip_non_colon(ad.ostate))) ],
            true)
            .find(ad => ad);
        if (!match_ad) {
            return;
        ...
        match_ad._attribute = thing.attribute(match_ad.istate) || thing.attribute(match_ad.ostate);
        ...
        if (match_ad.do) {
            ...
            resultd.ostate_previous = {};
            resultd.ostate_previous[match_ad._code] = thing.get(match_ad._code);  

            resultd.ostate = {};
            resultd.ostate[match_ad._code] = ovalue;
        }

        if (match_ad.response) {
            resultd.response = format.format(match_ad.response, { 
                name: thing.name(), // _.ld.first(thingd.meta, "schema:name"),
                value: thing.get(match_ad._code), // _.d.get(thingd.istate, match_ad._code),
            });
        }
