    ...
    const _level_2 = function(thingd) {
        ...
        if (match_ad.do) {
            do {
                ...
                match_header.header = match_header.header || match_ad.header || null;
            } while (false);
        }
        ...
        .subscribe(
            ...
            finished => {
                if (match_header.header) {
                    const header = match_header.header;
                    const count = matches.length - 0;
                    let template = header.response;

                    if ((count === 0) && header.zero) {
                        template = header.zero;
                    } else if ((count === 1) && header.one) {
                        template = header.one;
                    }
